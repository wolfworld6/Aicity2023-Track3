...
class FPN1D(nn.Module):
    ...
    def __init__(
        ...
        for i in range(self.start_level, self.end_level):
            ...
            l_conv = MaskedConv1D(in_channels[i], out_channel, 1, bias=False)
            # use depthwise conv here for efficiency
            fpn_conv = MaskedConv1D(out_channel, out_channel, 3, padding=1, bias=False, groups=out_channel)
    ...
    def forward(self, inputs, fpn_masks):
        ...
        for i in range(len(self.lateral_convs)):
            x, _ = self.lateral_convs[i](inputs[i + self.start_level], fpn_masks[i + self.start_level])
            laterals.append(x)
