...
class MaskedConv1D(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        B, C, T = x.size()                                  # [1, 2304, 2304]
        # input length must be divisible by stride
        assert T % self.stride == 0

        # conv
        out_conv = self.conv(x)                             # [1, 512, 2304]
        # compute the mask
        if self.stride > 1:
            ...
            out_mask = mask.float()                         # [1, 1, 2304]

        # masking the output, stop grad to mask
        out_conv = out_conv * out_mask.detach()             # [1, 512, 2304]
...
def ChannelAttention(x, num_heads=8, qkv_bias=False):      

    B, C, T  = x.shape                                                                      # [4, 512, 2304]
    head_dim = T // num_heads
    scale    = head_dim ** -0.5

    qkv_conv  = nn.Linear(T, T * 3, bias=qkv_bias, device = "cuda:0")
    # qkv_conv2 = nn.Linear(T * 3, T * 3, bias=qkv_bias, device = "cuda:0")
    proj      = nn.Linear(T, T, device = "cuda:0")

    qkv = qkv_conv(x)  
    # qkv = qkv_conv2(qkv)                                                                 # [4, 512, 6912]
    qkv = qkv.reshape(B, T, 3, num_heads, C // num_heads).permute(2, 0, 3, 1, 4)           # [3, 4, 8, 2304, 64]
    q, k, v = qkv[0], qkv[1], qkv[2]                                                       # [4, 8, 2304, 64]

    k = k * scale                                                                          # [4, 8, 2304, 64]
    attention = k.transpose(-1, -2) @ v                                                    # [4, 8, 64, 64]
    attention = attention.softmax(dim=-1)                                                  # [4, 8, 64, 64]
    x = (attention @ q.transpose(-1, -2)).transpose(-1, -2)                                # [4, 8, 2304, 64]
    x = x.transpose(1, 2).reshape(B, C, T)                                                 # [4, 512, 2304]
    # x = self.proj(x)
    return x

def ChannelAttentionv2(x, out, num_heads=4, qkv_bias=False):      
    ...
    qkv_conv  = nn.Linear(T, T * 3, bias=qkv_bias, device = "cuda:0")
    # qkv_conv2 = nn.Linear(T * 3, T * 3, bias=qkv_bias, device = "cuda:0")
    proj      = nn.Linear(T, Ta, device = "cuda:0")
    ...
    x = proj(x)
    return x
def ChannelAttention_window(x, num_chunks=8, window_overlap=4, head_dim=128, qkv_bias=False):                               # [4, 575, 8, 8]    
    
    x = x.view(x.size(0), num_chunks, -1)
    B, C, T  = x.shape                                                                     # [4, 512, 2304]
    scale    = head_dim ** -0.5
    ...
    qkv = qkv_conv(x)                                                                      # [4, 512, 6912]
    # pdb.set_trace()
    qkv = qkv.reshape(B, T, 3, num_chunks, C // num_chunks).permute(2, 0, 3, 1, 4)         # [3, 4, 8, 2304, 64]
    q, k, v = qkv[0], qkv[1], qkv[2]                                                       # [4, 8, 2304, 64]
    ...
    x = (attention @ q.transpose(-1, -2)).transpose(-1, -2)                                # [4, 8, 2304, 64]
    x = x.transpose(1, 2).reshape(B, C, T)                                                 # [4, 512, 2304]
    x = x.view(x.size(0), x.size(1), window_overlap*2, -1)
...
class LocalMaskedMHCA(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        assert n_embd % n_head == 0                                     # 512 / 4                                       
        self.n_embd = n_embd                                            # 512
        self.n_head = n_head                                            # 4
        self.n_channels = n_embd // n_head                              # 128  
        self.scale = 1.0 / math.sqrt(self.n_channels)                   # /dk 
        self.window_size = window_size                                  # 9
        self.window_overlap  = window_size // 2                       # 4
        # self.window_overlap  = window_size                              # 4
        ...
        assert (n_qx_stride == 1) or (n_qx_stride % 2 == 0)
        assert (n_kv_stride == 1) or (n_kv_stride % 2 == 0)
        self.n_qx_stride = n_qx_stride                                  # 1
        self.n_kv_stride = n_kv_stride                                  # 1
        # print(" ******************************** channel_att_sride: ",channel_att_sride)
        # pdb.set_trace()
        # self.channel_attention = ChannelAttention(2304 // channel_att_sride, n_head)
        # query conv (depthwise)
        kernel_size = self.n_qx_stride + 1 if self.n_qx_stride > 1 else 3 # 3
        stride, padding = self.n_kv_stride, kernel_size // 2              # 1, 1
        # 1d depthwise conv                                            
        ...
        # key, value conv (depthwise)
        kernel_size = self.n_kv_stride + 1 if self.n_kv_stride > 1 else 3 # 3
        stride, padding = self.n_kv_stride, kernel_size // 2              # 1,1
        # 1d depthwise conv
        # (conv): Conv1d(512, 512, kernel_size=(3,), stride=(1,), padding=(1,), groups=512, bias=False)
    ...
    def _pad_and_transpose_last_two_dims(x, padding):
        ...
        x = nn.functional.pad(x, padding)                                             # [4, 575, 9, 8]
        x = x.view(*x.size()[:-2], x.size(-1), x.size(-2))                            # [:-2]: [4, 575] , 8 , 9
        return x                                                                      # [4, 575, 8, 9]
    ...
    def _sliding_chunks_query_key_matmul(self, query, key, num_heads, window_overlap):
        ...
        """
        # query / key: B*nh, T, hs
        bnh, seq_len, head_dim = query.size()                                                   # 4, 2304, 128                                           
        batch_size = bnh // num_heads
        assert seq_len % (window_overlap * 2) == 0
        assert query.size() == key.size()

        chunks_count = seq_len // window_overlap - 1                                            # 575

        # B * num_heads, head_dim, #chunks=(T//w - 1), 2w
        chunk_query = self._chunk(query, window_overlap)                                        # [4, 2304, 128] ---> [4, 575, 8, 128]
        chunk_key   = self._chunk(key, window_overlap)                                          # [4, 2304, 128] ---> [4, 575, 8, 128]

        # matrix multiplication
        # bcxd: batch_size * num_heads x chunks x 2window_overlap x head_dim
        # bcyd: batch_size * num_heads x chunks x 2window_overlap x head_dim
        # bcxy: batch_size * num_heads x chunks x 2window_overlap x 2window_overlap
        diagonal_chunked_attention_scores = torch.einsum("bcxd,bcyd->bcxy", (chunk_query, chunk_key))                                                           # [4, 575, 8, 8]

        # diagonal_chunked_attention_scores_channel = ChannelAttention_window(diagonal_chunked_attention_scores, chunks_count, window_overlap, head_dim)          # [4, 575, 8, 8]  dxd       
        # # diagonal_chunked_attention_scores = 0.25 * diagonal_chunked_attention_scores_channel + 0.75 * diagonal_chunked_attention_scores                         # dxd
        # diagonal_chunked_attention_scores = diagonal_chunked_attention_scores_channel + diagonal_chunked_attention_scores                                     # dxd
        # pdb.set_trace()
        # convert diagonals into columns
        # B * num_heads, #chunks, 2w, 2w+1
        diagonal_chunked_attention_scores = self._pad_and_transpose_last_two_dims(diagonal_chunked_attention_scores, padding=(0, 0, 0, 1))                      # [4, 575, 8, 9]
        ...
        diagonal_attention_scores = diagonal_chunked_attention_scores.new_empty(
            (batch_size * num_heads, chunks_count + 1, window_overlap, window_overlap * 2 + 1)                      # [4, 576, 4, 9]
        )

        # copy parts from diagonal_chunked_attention_scores into the combined matrix of attentions
        # - copying the main diagonal and the upper triangle
        diagonal_attention_scores[:, :-1, :, window_overlap:] = diagonal_chunked_attention_scores[                  # [4, 576, 4, 5]
            :, :, :window_overlap, : window_overlap + 1
        ]
        diagonal_attention_scores[:, -1, :, window_overlap:] = diagonal_chunked_attention_scores[                   # [4, 4, 5]
            :, -1, window_overlap:, : window_overlap + 1
        ]
        # - copying the lower triangle
        diagonal_attention_scores[:, 1:, :, :window_overlap] = diagonal_chunked_attention_scores[                   # [4, 575, 4, 4]
            :, :, -(window_overlap + 1) : -1, window_overlap + 1 :
        ]

        diagonal_attention_scores[:, 0, 1:window_overlap, 1:window_overlap] = diagonal_chunked_attention_scores[    # [4, 3, 3]
            :, 0, : window_overlap - 1, 1 - window_overlap :
        ]

        # separate batch_size and num_heads dimensions again
        diagonal_attention_scores = diagonal_attention_scores.view(                                                # [1, 2304, 4, 9]
    ...
    def _sliding_chunks_matmul_attn_probs_value(self, attn_probs, value, num_heads, window_overlap):
        ...
        """
        bnh, seq_len, head_dim = value.size()                                                                    # [4, 2304, 128]
        batch_size = bnh // num_heads
        assert seq_len % (window_overlap * 2) == 0
        assert attn_probs.size(3) == 2 * window_overlap + 1                                                      # 9
        chunks_count = seq_len // window_overlap - 1                                                             # 575
        # group batch_size and num_heads dimensions into one, then chunk seq_len into chunks of size 2 window overlap

        chunked_attn_probs = attn_probs.transpose(1, 2).reshape(                                                 # [4, 576, 4, 9]
            batch_size * num_heads, seq_len // window_overlap, window_overlap, 2 * window_overlap + 1
        )

        # pad seq_len with w at the beginning of the sequence and another window overlap at the end
        padded_value = nn.functional.pad(value, (0, 0, window_overlap, window_overlap), value=-1)                # [4, 2312, 128]

        # pdb.set_trace()
        # chunk padded_value into chunks of size 3 window overlap and an overlap of size window overlap
        chunked_value_size = (batch_size * num_heads, chunks_count + 1, 3 * window_overlap, head_dim)            # (4, 576, 12, 128)        
        chunked_value_stride = padded_value.stride()                                                             # (295936, 128, 1)
        chunked_value_stride = (                                                                                 # (295936, 512, 128, 1)
            chunked_value_stride[0],
            window_overlap * chunked_value_stride[1],
            chunked_value_stride[1],
            chunked_value_stride[2],
        )
        chunked_value = padded_value.as_strided(size=chunked_value_size, stride=chunked_value_stride)            # [4, 576, 12, 128]

        chunked_attn_probs = self._pad_and_diagonalize(chunked_attn_probs)                                       # [4, 576, 4, 9] ---> [4, 576, 4, 12]                                  

        context = torch.einsum("bcwd,bcdh->bcwh", (chunked_attn_probs, chunked_value))                           # [4, 576, 4, 128]                          
        return context.view(batch_size, num_heads, seq_len, head_dim)                                            # [1, 4, 2304, 128]

    def _compute_global_attention_mask(attn_probs, chunked_attn_probs, window_overlap, before_sep_token=True):
        """
        Computes global attention mask by putting attention on all tokens before `sep_token_id` if `before_sep_token is
        True` else after `sep_token_id`.
            attention_mask = attention_mask * (global_attention_mask + 1)
        """
        question_end_index = _get_question_end_index(chunked_attn_probs, window_overlap)
        question_end_index = question_end_index.unsqueeze(dim=1)  # size: batch_size x 1
        # bool attention mask with True in locations of global attention
        attention_mask = torch.arange(attn_probs.shape[1], device=attn_probs.device)
        if before_sep_token is True:
            attention_mask = (attention_mask.expand_as(attn_probs) < question_end_index).to(torch.uint8)
        else:
            # last token is separation token and should not be counted and in the middle are two separation tokens
            attention_mask = (attention_mask.expand_as(attn_probs) > (question_end_index + 1)).to(torch.uint8) * (
                attention_mask.expand_as(attn_probs) < attn_probs.shape[-1]).to(torch.uint8)
    ...
    def forward(self, x, mask):
        # x: batch size, feature channel, sequence length,
        # mask: batch size, 1, sequence length (bool)
        B, C, T = x.size()                                                                  # [1, 512, 2304], mask:[1, 1, 2304]
        # step 1: depth convolutions
        # query conv -> (B, nh * hs, T')
        q, qx_mask = self.query_conv(x, mask)                                               # [1, 512, 2304], [1, 1, 2304]
        q = self.query_norm(q)
        # key, value conv -> (B, nh * hs, T'')
        k, kv_mask = self.key_conv(x, mask)                                                 # [1, 512, 2304], [1, 1, 2304]
        k = self.key_norm(k)
        v, _ = self.value_conv(x, mask)                                                     # [1, 512, 2304], [1, 1, 2304]
        v = self.value_norm(v)

        # step 2: query, key, value transforms & reshape
        # projections
        q = self.query(q)                                                                   # [1, 512, 2304]
        k = self.key(k)                                                                     # [1, 512, 2304]
        v = self.value(v)                                                                   # [1, 512, 2304]
        
        # (B, nh * hs, T) -> (B, nh, T, hs) n_head:4 n_channels:128
        q = q.view(B, self.n_head, self.n_channels, -1).transpose(2, 3)                     # [1, 4, 2304, 128]
        k = k.view(B, self.n_head, self.n_channels, -1).transpose(2, 3)                     # [1, 4, 2304, 128]
        v = v.view(B, self.n_head, self.n_channels, -1).transpose(2, 3)                     # [1, 4, 2304, 128]
        # view as (B * nh, T, hs)
        q = q.view(B * self.n_head, -1, self.n_channels).contiguous()                       # [4, 2304, 128]
        k = k.view(B * self.n_head, -1, self.n_channels).contiguous()                       # [4, 2304, 128]
        v = v.view(B * self.n_head, -1, self.n_channels).contiguous()                       # [4, 2304, 128]

        # step 3: compute local self-attention with rel pe and masking
        q *= self.scale
        # chunked query key attention -> B, T, nh, 2w+1 = window_size
        att = self._sliding_chunks_query_key_matmul(q, k, self.n_head, self.window_overlap) # [1, 2304, 4, 9]
        # rel pe
        if self.use_rel_pe:
            att += self.rel_pe
        # kv_mask -> B, T'', 1
        inverse_kv_mask = torch.logical_not(kv_mask[:, :, :, None].view(B, -1, 1))          # # [1, 2304, 1]
        ...
        diagonal_mask = self._sliding_chunks_query_key_matmul(                              # [1, 2304, 1, 9]
            float_inverse_kv_mask.new_ones(size=float_inverse_kv_mask.size()),
            float_inverse_kv_mask,
            1,
            self.window_overlap
        )
        att += diagonal_mask                                                                # [1, 2304, 4, 9]
        ...
        # chunked attn value product -> B, nh, T, hs                                       # train-8h: [4, 8, 2304, 64] test-4h:[1, 4, 2304, 128]
        out = self._sliding_chunks_matmul_attn_probs_value(att, v, self.n_head, self.window_overlap)
        # transpose to B, nh, hs, T -> B, nh*hs, T
        out = out.transpose(2, 3).contiguous().view(B, C, -1)                              # [1, 512, 2304]
        # g_att_mask = self._compute_global_attention_mask(att, out, self.window_overlap)
        # print("out: ", out.size())
        # att_channel = ChannelAttention(out, self.n_head)                                   # dxd
        # print("att_channel: ", att_channel.size())
        # out = out + att_channel                                                            # dxd approach 1
        # # out = att_channel                                                                # dxd approach 1

        # att_channel = ChannelAttention(out, self.n_head)
        # out = 0.75 * out + 0.25 * att_channel                                            # dxd approach 2
        # # output projection + skip connection
        
        # print("out : ", out.shape)
        # att_channel = ChannelAttentionv2(x, out, self.n_head)  
        # print("att_channel: ", att_channel.shape)                              
        # out = out + att_channel                                                            
        # # out = 0.75 * out + 0.25 * att_channel                                               # dxd approach 3
        
        # output projection + skip connection        
        out = self.proj_drop(self.proj(out)) * qx_mask.float()
        return out, qx_mask                                                                # [1, 512, 2304],[1, 1, 2304]
...
class TransformerBlock(nn.Module):
    ...
    def __init__(
        self,
        n_embd,                # dimension of the input features                              # 512
        ...
        path_pdrop=0.0,        # drop path rate                                               # 0.1
        mha_win_size=-1,       # > 0 to use window mha                                        # 9
        ...
        if mha_win_size > 1:                                                                  # 9
            self.attn = LocalMaskedMHCA(
                n_embd,                                                                       # 512
                n_head,                                                                       # 4
                window_size=mha_win_size,                                                     # 9
                n_qx_stride=n_ds_strides[0],                                                  # 1
                n_kv_stride=n_ds_strides[1],                                                  # 1
                channel_att_sride=channel_att_sride,
                attn_pdrop=attn_pdrop,
                proj_pdrop=proj_pdrop,
                use_rel_pe=use_rel_pe  # only valid for local attention                       # false
            )
        else:
        ...
        # two layer mlp
        if n_hidden is None:
            n_hidden = 4 * n_embd  # default                                                  # 2048
        if n_out is None:
            n_out = n_embd                                                                    # 512
        ...
        # drop path
        if path_pdrop > 0.0:                                                                 # 0.1
            self.drop_path_attn = AffineDropPath(n_embd, drop_prob = path_pdrop)
            self.drop_path_mlp = AffineDropPath(n_out, drop_prob = path_pdrop)
        else:
            self.drop_path_attn = nn.Identity()
            self.drop_path_mlp = nn.Identity()
        # jump backbone 72 for idx in range(arch[1]):
    def forward(self, x, mask, pos_embd=None):                                               # x0: [1, 512, 2304], mask0: [1, 1, 2304]
        # pdb.set_trace()
        # pre-LN transformer: https://arxiv.org/pdf/2002.04745.pdf
        out, out_mask = self.attn(self.ln1(x), mask)                                         # x0: [1, 512, 2304], mask0: [1, 1, 2304] ([[[ True,  True,  True,  ..., False, False, False]]])
        out_mask_float = out_mask.float()                                                    # [1, 1, 2304]
        out = self.pool_skip(x) * out_mask_float + self.drop_path_attn(out)                  # [1, 512, 2304]               
        # FFN
        out = out + self.drop_path_mlp(self.mlp(self.ln2(out)) * out_mask_float)             # [1, 512, 2304]
        # optionally add pos_embd to the output
        if pos_embd is not None:
            out += pos_embd * out_mask_float
        return out, out_mask
        # jump backbones.py(135)
...
class PoolFormerEmbeddings(nn.Module):
    ...
    def __init__(self, hidden_size, num_channels, patch_size, stride, padding):
        super().__init__()
...
class PoolFormerOutput(nn.Module):
    def __init__(self, act_layer, dropout_prob, hidden_size, intermediate_size):
        super().__init__()
...
class PoolFormerLayer(nn.Module):
    ...
    def __init__(self, num_channels, pool_size, hidden_size, intermediate_size, drop_path, stride, act_layer=nn.GELU, 
        use_layer_scale=True, layer_scale_init_value=1e-5):
        super().__init__()
        self.pooling = PoolFormerPooling(pool_size)
        self.output = PoolFormerOutput(act_layer, drop_path, hidden_size, intermediate_size)
        ...
        if use_layer_scale:
            self.layer_scale_1 = nn.Parameter(
                layer_scale_init_value * torch.ones((num_channels)), requires_grad=True)
