...
class ANETdetection(object):
    ...
    def evaluate(self, preds, verbose=True):
        ...
        # self.activity_index = {1: 0, 2: 1, 3: 2, 4: 3, 5: 4, 6: 5, 7: 6, 8: 7, 9: 8, 10: 9, 11: 10, 12: 11, 13: 12, 14: 13}
        # make the label ids consistent
        preds['label'] = preds['label'].replace(self.activity_index)
        # preds.to_csv('predict_info_replace_predict_05_original.csv', mode='w')
        # preds.to_csv('action_predict_info_singleview_original.csv', mode='w')
        preds.to_csv('action_predict_info_singleview_original_05-09.csv', mode='w')
        # preds.to_csv('predict_info_replace_predict_05_original_with-cls-prob.csv', mode='w')
        # compute mAP
