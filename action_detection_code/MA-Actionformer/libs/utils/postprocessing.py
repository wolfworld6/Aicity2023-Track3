...
def load_results_from_npz(filepath):
    ...
    for npz_file in filelist:
        ...
        npz_name = re.findall(r'[^\\/:*?"<>|\r\n]+$', npz_file)[0][:-4]
        feats_prob = np.load(os.path.join(filepath, npz_file))['a_prob'].astype(np.float32)
        all_clip_scores = []
        for clip_idx in range(feats_prob.shape[0]):
            clip_feat_score = feats_prob[clip_idx]
            all_clip_scores.append(clip_feat_score)
        prob_dict[npz_name] = all_clip_scores
    # pdb.set_trace()
    return prob_dict
...
def postprocess_results(results, cls_score_file, num_pred=200, topk=2):
    ...
    elif isinstance(cls_score_file, str):
        cls_scores = load_results_from_npz(cls_score_file)                                  # all_clips_by_video_id
    else:
    ...
    # process each video
    for vid, result in results.items():                                 # ori json:'wONwHYy59Tc'
        # pick top k cls scores and idx
        pdb.set_trace()
        curr_cls_scores = np.asarray(cls_scores[vid])                   # ori cls_scores['wONwHYy59Tc'] -> list 
        topk_cls_idx = np.argsort(curr_cls_scores)[::-1][:topk]         # 从小到大排序后取反，从大到小，再取topk
        ...
        # K x 1 @ 1 N -> K x N -> KN
        # multiply the scores
        new_pred_score = np.sqrt(topk_cls_score[:, None] @ pred_score[None, :]).flatten()   # topk:1 x 2 -> 2 x 1, pred: 200 x 1 -> 1 x 200 -> 400 x 1
        new_pred_segment = np.tile(pred_segment, (topk, 1))                                 # pred_segment : (200, 2) -> copy pred_seg (400, 2)
