...
def main(args):
    ...
    # model_ema = ModelEma(model, decay=0.75)
    ...
    if args.pretrain:
        checkpoint = torch.load(args.pretrain, map_location = lambda storage, loc: storage.cuda(cfg['devices'][0]))
        model.load_state_dict(checkpoint['state_dict'], strict=False)
        model_ema.module.load_state_dict(checkpoint['state_dict_ema'], strict=False)
        print('Load pretrain model from ', args.pretrain)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrain', default='', type=str, metavar='PATH', help='path to a checkpoint (default: none)')
