...
MODEL_PATH = '/mnt/home/video_action/UniFormerV2-main/extract_clip/clip_vit_checkpoint'
_MODELS = {
...
class Transformer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.frozen:
            ...
            # x = self.proj[0](cls_token[0, :, :])
            # x_feature = self.proj[1](x)
            # x_pred = self.proj[2](x_feature)
            ...
            # weight = self.sigmoid(self.balance)
            # residual = x.view(L, N, T_down, C)[0].mean(1)  # L, N, T, C
            # x = self.proj[0]((1 - weight) * cls_token[0, :, :] + weight * residual)
            # x_feature = self.proj[1](x)
            # x_pred = self.proj[2](x_feature)
            # return x_feature,x_pred   
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        return out
        # out_feature,out_pred = self.transformer(x)
        # return out_feature,out_pred
