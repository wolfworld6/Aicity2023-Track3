...
class Kinetics_sparse(torch.utils.data.Dataset):
                ...
                for idx in range(self._num_clips):
                    if self.mode=='train':
                        self._path_to_videos.append(
                            os.path.join('/AIcityTrack3','croped_splited_videos', path)
                        )
                    else:
                        self._path_to_videos.append(
                            os.path.join('/AIcityTrack3','croped_splited_videos_A2', path)
                        )                    
    ...
    def __getitem__(self, index):
        ...
        for i_try in range(self._num_retries):
            ...
            # #手动裁剪，padding，做增强和randcrop之前 THWC
            # frames = frames[:,int(frames.shape[1]*0.1):,int(frames.shape[2]/5*1):,:]
            #                 # T H W C
            # frames = torch.nn.functional.pad(frames,(0,0,0,0,int((frames.shape[2]-frames.shape[1])/2),int((frames.shape[2]-frames.shape[1])/2)),"constant",0) #
            # # print(frames.shape)
            # # print("==================")
